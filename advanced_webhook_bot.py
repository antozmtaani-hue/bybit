...
- Performance and scalability improved with async APIs and event-driven design
"""
...
async def bybit_private_request_async(method: str, endpoint: str, api_key: str, api_secret: str,
                                      base_url: str, params: Dict = None, session: aiohttp.ClientSession = None) -> Optional[Dict]:
    try:
        ...
        async with session.request(method, url, headers=headers, data=body_str if method.upper() != 'GET' else None, timeout=10) as resp:
            resp.raise_for_status()
            return await resp.json()
    except Exception as e:
        logging.error(f"Bybit API async error: {e}")
        return None


async def get_market_price_async(symbol: str, base_url: str, session: aiohttp.ClientSession) -> Optional[float]:
    try:
        async with session.get(f"{base_url}/v5/market/tickers", params={'category': 'spot', 'symbol': symbol}, timeout=10) as resp:
            resp.raise_for_status()
        ...
        # fallback to coingecko api for mapping symbols
        coingecko_map = {
            "BNBUSDT": "binancecoin",
            "ETHUSDT": "ethereum",
        ...
        if coin_id:
            async with session.get("https://api.coingecko.com/api/v3/simple/price",
                                   params={"ids": coin_id, "vs_currencies": "usd"}, timeout=10) as cg_resp:
        ...
        logging.error(f"Async error getting market price for {symbol}: {e}")
        return None


async def get_available_balance_async(asset: str, api_key: str, api_secret: str, base_url: str,
                                      session: aiohttp.ClientSession) -> float:
    try:
        params = {'accountType': 'SPOT'}
        data = await bybit_private_request_async('GET', '/v5/account/wallet-balance', api_key, api_secret, base_url, params, session)
        ...
        logging.error(f"Failed to fetch balance for {asset} async: {e}")
        return 0.0


async def place_market_order_async(symbol: str, side: str, size: float,
    ...
    try:
        ...
        market_resp = await bybit_private_request_async('POST', '/v5/order/create',
                                                       api_key, api_secret, base_url, market_params, session)
        if not market_resp or market_resp.get('retCode') != 0:
            logging.error(f"Failed to place market order async: {market_resp}")
            return None
        return market_resp
    except Exception as e:
        logging.error(f"Error placing market order async: {e}")
        return None


def check_webhook_api_sync(webhook_token: str) -> Optional[Dict]:
    # Keep synchronous because webhook.site API used and simplicity
    try:
        import requests
...
async def cross_check_trade_async(order_id: str, symbol: str, api_key: str,
                                  api_secret: str, base_url: str, session: aiohttp.ClientSession) -> bool:
    try:
        params = {'category': 'spot', 'orderId': order_id, 'symbol': symbol}
        resp = await bybit_private_request_async('GET', '/v5/order/realtime',
                                                api_key, api_secret, base_url, params, session)
        ...
        logging.error(f"Error cross-checking trade async: {e}")
        return False


async def price_monitor_process(trade_queue: Queue, stop_event: Event, config: Dict):
    ...
    async with aiohttp.ClientSession() as session:
        while not stop_event.is_set():
            try:
                ...
                for trade_id, trade in active_trades.items():
                    ...
                    if trade['monitoring'] and (current_time - trade.get('last_check', 0)) >= 10:
                        ...
                        current_price = await get_market_price_async(trade['symbol'], base_url, session)
                        if current_price:
                            logging.info(f"Trade {trade_id}: Current price ${current_price:.6f}, Entry price ${trade['entry_price']:.6f}")
                            # Calculate SL/TP on the fly
                            ...
                            if close_reason:
                                ...
                                close_resp = await bybit_private_request_async('POST', '/v5/order/create',
                                                                               api_key, api_secret, base_url, close_params, session)
                    ...
                    order_resp = await bybit_private_request_async('GET', '/v5/order/history',
                                                                   api_key, api_secret, base_url, order_params, session)
            ...
            # Instead of fixed sleep, wait asynchronously for 1 second to yield control
...
def webhook_monitor_process(signal_queue: Queue, config: Dict, stop_event: Event):
    ...
    while not stop_event.is_set():
        try:
            webhook_data = check_webhook_api_sync(webhook_token)
            if webhook_data:
...
async def trade_processor_main(signal_queue: Queue, trade_queue: Queue, config: Dict, stop_event: Event):
        ...
        while not stop_event.is_set():
            try:
                if not signal_queue.empty():
                    ...
                    current_price = await get_market_price_async(symbol, base_url, session)
                    ...
                    if action == 'BUY':
                        balance = await get_available_balance_async(quote_asset, api_key, api_secret, base_url, session)
                        trade_amount = balance * use_balance_pct
                        position_size = trade_amount / current_price
                    else:
                        balance = await get_available_balance_async(base_asset, api_key, api_secret, base_url, session)
                    ...
                    market_order = await place_market_order_async(
                        symbol, action, position_size,
                        api_key, api_secret, base_url, session
                    )

                    if market_order:
                        order_id = market_order.get('result', {}).get('orderId')
                        if await cross_check_trade_async(order_id, symbol, api_key, api_secret, base_url, session):
...
def main():
    ...
    # Use multiprocessing to run async event loop in trade processor subprocess
    def trade_proc_runner(signal_queue, trade_queue, config, stop_event):
        asyncio.run(trade_processor_main(signal_queue, trade_queue, config, stop_event))

    trade_process = Process(
        target=trade_proc_runner,
        args=(signal_queue, trade_queue, config, stop_event),
        daemon=True
    )

    # Price monitor runs async in separate subprocess
    def price_monitor_runner(trade_queue, stop_event, config):
        asyncio.run(price_monitor_process(trade_queue, stop_event, config))

    price_monitor = Process(
        target=price_monitor_runner,
    ...
    def signal_handler(sig, frame):
        ...
        price_monitor.join(timeout=5)
        for p in [webhook_process, trade_process, price_monitor]:
            if p.is_alive():
                p.terminate()
